...
def n_over_Z_kappa_scalar(Bi, Bb, Be, kappa, T_anisotropy):
    k = mp.mpf(kappa)
...
def n_over_Z_kappa_array_parallel(Bi_array, Bb, Be, kappa, T_anisotropy, n_jobs=-1):
    results = Parallel(n_jobs=n_jobs, backend="loky")(
        delayed(n_over_Z_kappa_scalar)(Bi, Bb, Be, kappa, T_anisotropy)
        for Bi in Bi_array
    )
...
for iT, T_anisotropy in enumerate(T_anisotropy_list):
    n_bimax[iT] = n_over_Z_biMaxwell(Bi_grid, Bb, Be, T_anisotropy)
