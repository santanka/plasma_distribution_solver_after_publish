...
def Pperp_over_ZTperp_kappa_scalar(Bi, Bb, Be, kappa, T_anisotropy):
    ...
    term1 = hyp2f1(k+mp.mpf('0.5'), 2, 2*k+mp.mpf('1.5'), z1) * k * (k-1) * (Bi/Bb)**2 * mp.gamma(2*k-mp.mpf('0.5')) / mp.gamma(2*k+mp.mpf('1.5'))


    # --- 第2項 ---
    pref2 = 1. / mp.sqrt(mp.pi) * (k - mp.mpf('0.5'))**2 * k / (k - 1) * mp.gamma(k+1) / mp.gamma(k+mp.mpf('0.5')) / T_anisotropy**2 * (Bi/(Be-Bb))**2
    ...
    def integrand(z):
        ...
        return z * w * (A**(-(k+1))) * (B**(-(k+mp.mpf('0.5')))) * incomplete_beta_Bz(mp.mpf('0.5'), k+mp.mpf('0.5'), zlim)
...
for iT, T_anisotropy in enumerate(T_anisotropy_list):
    ...
    ax.set_ylabel(r"$P_{\perp} / Z T_{\perp b}$")
